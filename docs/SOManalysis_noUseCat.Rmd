---
title: <font size = "5"> Excludes use variables - Self-organizing map analysis of spring runoff from Mt Mansfield </font>
...
This version excludes the variables quanitying the water diverted by the ski resort ("use" variables)
<br><br>
...
# Create a new folder to save the plots and CSVs produced by the SOM code
  newFolder <- "data/somResults/noUseCat/"
...
```{r select vars, include = TRUE, echo = FALSE, warning = FALSE, message = FALSE, fig.width = 8, out.width = "80%"}
...
# Select vars to keep Step 1
  ...
  drop.info <- names(mets_sub_1site %>% 
                       select(# Remove the non-numerical ID/INFO
                              ...
                              q_mm, q_mm_cum,
                              # Remove other variables we know we won't want
                              temp_max, temp_min, wind_spd_avg,
                              # We'll keep the average temps measured at VWC/FEMC station, so drop those from Morrisville/Stowe airport
                              temp_avg,
                              # Let's also remove the use categories
                              starts_with("use")))
...
Looks like we won't need to remove any variables based on correlations
<br><br>
...
params <- read_csv(here("Data", "somParams", "params_SOM_days180to245.csv"))
...
for (i in 1:nrow(params)) { # original for loop starter
      ...
      newFolder2 <- paste("Data/somResults/noUseCat/", Sys.Date(), "/SOMplots_PDFs", sep = "")
...
# Manually choose the runs you want
best_summ_top <- 
  ResultsDF %>% 
  select(Run, rows, cols, Nodes, Clusters, npF, QE) %>% 
  # Choose runs based on run #
  filter(Run %in% c(7, 20, 16))
...
# CREATE DF WITH EVENT IDs & CLUSTER #'s ----  
  ...
  datWithCluster_3cl <- clustAssignmDF %>% 
    ...
    # Arrange columns
    select(site, wyear, date, doy_wyear, cluster, q_mm, q_mm_cum, everything())
...
##### 3 clusters - examine boxplots of independent variables by cluster
```{r boxplots 3cl, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.asp = 0.8}
# BOXPLOTS OF INDEPENDENT VARIABLES BY CLUSTER ----
  datWithCluster_3cl %>%
    # Remove the drop.info vars from above
    select(-one_of(drop.info)) %>% 
    pivot_longer(cols = snow_dep_m:ncol(.), names_to = "var", values_to = "value") %>% 
...
datWithCluster_3cl %>%     
  ...
  mutate_at(vars(c(snow_dep_m:ncol(.))),
...
#### 3 clusters - examine how the clusters map on to the cumulative runoff curves
...
##### 4 clusters - examine boxplots of independent variables by cluster
```{r boxplots 4cl, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.asp = 0.8}
# BOXPLOTS OF INDEPENDENT VARIABLES BY CLUSTER ----
  datWithCluster_4cl %>%
    # Remove the drop.info vars from above
    select(-one_of(drop.info)) %>% 
...
#### 4 clusters - examine how the clusters map on to the cumulative runoff curves
```{r, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.asp = 1, out.width = "90%"}
datWithCluster_4cl %>% 
  mutate(cluster = factor(cluster)) %>% 
  ggplot(aes(x = date)) +
...
```{r boxplots 5cl, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.asp = 0.8}
# BOXPLOTS OF INDEPENDENT VARIABLES BY CLUSTER ----
  datWithCluster_5cl %>%
    # Remove the drop.info vars from above
    select(-one_of(drop.info)) %>% 
